...
попыток сделано:      10 → 18                     ← усилия red-team видны
ложные блокировки:    0% → 0%                     [SC-004: цель ≤10%]
контролей пройдено:   6/11 → 10/11                (+ ACC-02 информационный)
```

> ⚠️ **Как читать это число.** Доля считается от **исходных атак набора**, а не от числа попыток —
> иначе red-team, который старается сильнее, «улучшал» бы оценку защиты (так и было до фичи 003).
> И главное: набор атак и правила детекции написаны внутри проекта, поэтому 80% характеризуют
> **внутреннюю согласованность стенда, а не робастность против внешнего атакующего**. Подробнее —
> [specs/003-evaluation-honesty/baseline-numbers.md](specs/003-evaluation-honesty/baseline-numbers.md).
...
- **✅ covered** — есть реальный механизм (привязан к контролю scorecard); сейчас 3;
- **🔧 achievable** — достижимо кодом, в бэклоге (14);
