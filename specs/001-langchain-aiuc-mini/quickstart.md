...
пробили:            5/6  ->  1/6
доля успешных атак: 83%  ->  17%   (снижение 80%)  [SC-003: цель ≥70% — PASS]
попыток сделано:    10   ->  18                    ← усилия red-team
ложные блокировки:  0%   ->  0%                    [SC-004: цель ≤10% — PASS]
контролей пройдено: 6/11 ->  10/11                 (+ ACC-02 информационный)
```

> Цифры уточнены фичей 003: доля считается от **исходных атак**, а не от попыток; в счёт идут
> только контроли, способные провалиться. Показатель характеризует внутреннюю согласованность
> стенда, а не робастность против внешнего атакующего.
