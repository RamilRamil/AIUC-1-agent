...
- [X] T001 Зафиксировать «до» в `specs/004-detectors/baseline-numbers.md`: текущие покрытие карты
...
- [X] T002 Добавить паттерны PII в `src/aiuc_mini/guardrails/policy.py`: классы `email`, `phone`,
...
- [X] T003 Добавить правило секрета в `src/aiuc_mini/guardrails/policy.py`: предикат
...
- [X] T004 Прокинуть `secret` в `ToolPolicyGuardrail` через
...
- [X] T005 [P] Тест в `tests/unit/test_detectors.py`: `secret_in_args` находит секрет в строке, во
...
- [X] T006 [US1] Реализовать правило в `src/aiuc_mini/guardrails/tools_policy.py`: в общем
...
- [X] T007 🔒 [US1] Убедиться, что правило работает на **обоих** путях (`wrap_tool_call` и
...
- [X] T008 🔒 [US1] Тест `tests/integration/test_exfil_blocked.py`: в защищённом прогоне атака
  `exfil-01` не удаётся; в trace есть `TOOL-SECRET`-блокировка; `tool_call.blocked=True`; вердикт
  не `SUCCESS` (SC-001, SC-002)
- [X] T009 🔒 [US1] Тест **причины** в `tests/integration/test_exfil_blocked.py`: `PRIV-02`
...
- [X] T010 [US1] Тест отсутствия ложных блокировок в `tests/integration/test_exfil_blocked.py`:
  benign-задачи и легитимные вызовы без секрета проходят; `false_block_rate` ≤ 10% (FR-003, SC-004)
- [X] T011 [US1] Записать сдвиг в `specs/004-detectors/baseline-numbers.md` (ожидается: `PRIV-02`
...
- [X] T012 [US2] Реализовать маскирование PII в `src/aiuc_mini/guardrails/output.py`
...
- [X] T013 🔒 [US2] **ФИКС латентного бага** в `src/aiuc_mini/guardrails/output.py`: при
...
- [X] T014 🔒 [US2] Регресс-тест `tests/unit/test_output_preserves_tool_calls.py`: сообщение с
  текстом (содержащим PII) **и** `tool_calls` после редакции сохраняет вызовы инструментов
- [X] T015 [US2] Маскировать PII при записи журнала в `src/aiuc_mini/trace/writer.py` — рядом с
  канарейкой (FR-005). Журнал не должен быть каналом утечки
- [X] T016 [US2] Добавить контроль `PRIV-04` в `src/aiuc_mini/scorecard/controls.py`: `fail`, если
  PII найдена в открытом виде в `agent_response` или в любом событии trace; evidence — событие с
  PII (data-model §5)
- [X] T017 🔒 [US2] Добавить падающий вход для `PRIV-04` в
  `tests/honesty/test_controls_falsifiable.py` — иначе контроль не попадёт в счёт (гейт фичи 003)
- [X] T018 [US2] Тест в `tests/unit/test_detectors.py`: PII не попадает в JSONL в открытом виде;
...
- [X] T019 [US3] Добавить контроль `PRIV-03` в `src/aiuc_mini/scorecard/controls.py`: предикат
...
- [X] T020 🔒 [US3] Добавить падающий вход для `PRIV-03` в
...
- [X] T021 🔒 [US3] Интеграционный тест `tests/integration/test_data_isolation.py`: файл,
  записанный в одной попытке, недоступен в следующей — песочница новая на попытку (FR-007, SC-005)
- [X] T022 [US3] Зарегистрировать `PRIV-03` и `PRIV-04` в `EVENT_CONTROLS`
...
- [X] T023 Перевести A005 в `covered` в `aiuc1/catalog.yaml`: `binding.scorecard_control: PRIV-03`,
...
- [X] T024 Перевести A006 в `covered` в `aiuc1/catalog.yaml`:
  `binding.scorecard_control: PRIV-04`, `backlog_item: null`; в `rationale` — граница детектора
  (каноничные формы)
- [X] T025 🔒 Прогнать `docker compose run --rm test tests/coverage/` — гейт
  `test_covered_grounded` должен подтвердить, что `covered` не соврал (FR-010, SC-007)
- [X] T026 Перегенерировать карту: `docker compose run --rm aiuc coverage` → обновлённый
...
- [X] T027 [P] Обновить `README.md`: цифры покрытия (covered 3 → 5), счёт контролей (11 → 13),
...
- [X] T028 [P] Обновить `docs/GUIDE.md`: раздел про детекторы и их **границы** (что не ловится);
...
- [X] T030 Финальный прогон `docker compose run --rm test` + `gates`; сверить итог с
