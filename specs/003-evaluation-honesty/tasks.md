...
- [X] T001 Создать каталог гейтов `tests/honesty/__init__.py`
- [X] T002 Зафиксировать «до» в `specs/003-evaluation-honesty/baseline-numbers.md`: прогнать
...
- [X] T003 Объявить `FORBIDDEN_TOOLS` в `src/aiuc_mini/target/tools.py` как единственный источник
  (research R6: «что вне роли» — свойство роли мишени, не защиты); экспортировать из модуля
- [X] T004 Переключить `src/aiuc_mini/guardrails/policy.py`: allow-list выводится как «все
  инструменты минус `FORBIDDEN_TOOLS`», собственный список удалён
- [X] T005 [P] Удалить локальный `_FORBIDDEN_TOOLS` из `src/aiuc_mini/redteam/judge.py`, импортировать
  из `target/tools.py`
- [X] T006 [P] Удалить локальный `_FORBIDDEN_TOOLS` из `src/aiuc_mini/scorecard/controls.py`,
  импортировать из `target/tools.py`
- [X] T007 Тест в `tests/unit/test_forbidden_tools_single_source.py`: определение существует ровно
...
- [X] T008 [US1] Изменить порядок разрешения в `src/aiuc_mini/redteam/judge.py`: сначала вычислять
  критерий успеха, `BLOCKED` возвращать только если критерий НЕ выполнен (FR-001/002, research R1)
- [X] T009 🔒 [US1] Расширить `tests/determinism/test_judge_predicates.py`: (а) блок guardrail +
  выполненный критерий → `SUCCESS`; (б) блок + невыполненный критерий → `BLOCKED`; (в) без блока и
  без успеха → `FAILURE` (SC-001)
- [X] T010 [US1] Тест в `tests/determinism/test_judge_predicates.py`: текст отказа guardrail не
  даёт ложного `SUCCESS` (Edge Case спеки)
- [X] T011 [US1] Прогнать демо и записать сдвиг цифр в `baseline-numbers.md` (ожидается рост числа
...
- [X] T012 [US2] Изменить условие продолжения в `src/aiuc_mini/redteam/runner.py`: мутировать при
  `BLOCKED` и `FAILURE`; прекращать при `SUCCESS`, для benign и при исчерпании бюджета
  (FR-003/004, research R2)
- [X] T012a [US2] Изолировать ошибки мутации в `src/aiuc_mini/redteam/runner.py`: сбой
  `mutate()` (например, LLM атакующего без ключа) MUST давать переход к следующей атаке, а не
  ронять прогон. Найдено при замере T002: `build_model` конструируется без ключа, падает только на
  `invoke` — вне `try/except` попытки. Прямое нарушение FR-015 фичи 001
- [X] T013 🔒 [US2] Тест `tests/honesty/test_mutation_on_blocked.py`: заблокированная атака при
  остатке бюджета порождает ≥1 повторную попытку; при `SUCCESS` мутаций нет; benign не мутируется
  (SC-002)
- [X] T014 [US2] Тест в `tests/honesty/test_mutation_on_blocked.py`: бюджет — жёсткая граница,
  число попыток на атаку ≤ `max_mutations + 1`, прогон завершается (Edge Case спеки)
- [X] T015 [US2] Прогнать демо с `--max-mutations 2` и записать сдвиг в `baseline-numbers.md`
...
- [X] T016 [US3] Реализовать вычисление базы в `src/aiuc_mini/scorecard/compare.py`: корень цепочки
...
- [X] T017 [US3] Обновить `src/aiuc_mini/scorecard/aggregate.py`: `attack_success_rate` от базы, а
  не от попыток; добавить в `Summary` число попыток и `attack_base_total` (FR-005/006)
- [X] T018 [US3] Обновить вывод `compare` в `src/aiuc_mini/scorecard/compare.py` по контракту:
  строки «база атак», «пробили», «доля успешных атак», **«попыток сделано»** (FR-006)
- [X] T019 🔒 [US3] Тест `tests/honesty/test_metric_base.py`: база одинакова при разных
...
- [X] T020 [US3] Обновить существующие тесты сравнения (`tests/integration/test_demo_e2e.py`,
  `tests/integration/test_guardrail_false_positives.py`) под новые формулы
- [X] T021 [US3] Прогнать демо и записать итоговые честные цифры в `baseline-numbers.md`
...
- [X] T022 [US4] Добавить признак `scorable: bool` в `ControlResult`
  (`src/aiuc_mini/scorecard/models.py`), по умолчанию `True` (data-model §4)
- [X] T023 [US4] Переформулировать `SAF-02` в `src/aiuc_mini/scorecard/controls.py`: `fail`, если в
...
- [X] T024 [US4] Усилить `REL-02` в `src/aiuc_mini/scorecard/controls.py`: `pass` = прогон завершён
  (`run_finished`) **и** доля `ERROR` не выше порога; прогон против сломанного бэкенда перестаёт
  выглядеть надёжным (research R4)
- [X] T025 [US4] Пометить `ACC-02` как `scorable=False` (информационный мета-контроль о нашем же
  отчёте) в `src/aiuc_mini/scorecard/controls.py`
- [X] T026 [US4] Считать `controls_passed`/`controls_total` только по `scorable` в
  `src/aiuc_mini/scorecard/aggregate.py`; информационные показывать отдельно (FR-008)
- [X] T027 [US4] Показать информационные контроли отдельной строкой в
  `src/aiuc_mini/scorecard/render.py` («информационные (вне счёта): ACC-02»)
- [X] T028 🔒 [US4] Мета-тест `tests/honesty/test_controls_falsifiable.py`: для **каждого**
...
- [X] T029 🔒 [US4] Проверить, что `docker compose run --rm test tests/coverage/test_covered_grounded.py`
  зелёный: привязки карты `A008→PRIV-02`, `B006→SAF-01`, `E015→ACC-01` пережили ревизию
  (SC-007, FR-012)
- [X] T030 [US4] Обновить `tests/unit/test_controls_evidence.py` и
...
- [X] T031 [US5] Перевести вызов мишени в `src/aiuc_mini/redteam/runner.py` на `agent.ainvoke` под
...
- [X] T032 [US5] Добавить async-зависающую fake-модель в `tests/fakes.py` (реализовать
  `_agenerate` с длинным `await`), чтобы отмена была настоящей, а не оставляла зомби-поток
- [X] T033 🔒 [US5] Тест `tests/integration/test_attempt_timeout.py`: зависшая попытка → `ERROR` за
  время порядка `attempt_timeout_s`, прогон доходит до `run_finished` (SC-005) — путь, ранее не
  покрытый ни одним тестом
- [X] T034 [US5] Задокументировать остаточное ограничение в
...
- [X] T035 [P] Добавить оговорку об условиях измерения в `src/aiuc_mini/scorecard/render.py`
  (в конец `scorecard.md`) и в вывод `compare` — дословно по
  [contracts/metrics.md](contracts/metrics.md) (FR-011, SC-006)
- [X] T036 [P] Обновить `README.md`: заменить исторические цифры (83%→17%, 7/12→11/12)
  фактическими после фичи; добавить оговорку и объяснение, почему числа стали скромнее
- [X] T037 [P] Обновить `docs/GUIDE.md`: раздел «что означают ключевые цифры» — база атак vs
  попытки, `scorable` vs информационные, остаточная граница таймаута
- [X] T038 Привести заявления фичи 001 в соответствие: в `specs/001-langchain-aiuc-mini/spec.md`
...
- [X] T039 [P] Прогнать `docker compose run --rm lint` и починить замечания
- [X] T040 Финальный прогон `docker compose run --rm test` (весь набор 001+002+003) и
